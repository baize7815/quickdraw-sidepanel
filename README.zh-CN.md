...
| 图片处理 | 裁剪、遮罩、宫格切图、透明轮廓转矢量 | 少见，或要导出后再处理 | 通常没有 |
...
| 处理图片 | 自由与比例裁剪、旋转、镜像、宫格切图、图片与形状遮罩 |
...
- **比例裁剪：** 单击裁剪按钮进入自由裁剪，双击选择比例，支持 `1:1`、`9:16`、`16:9`、`3:4`、`4:3` 和自定义比例。
- **宫格切图：** 按行列切成独立图片，最多 100 块，可以分别移动与导出。
